...
template <typename T>
static void
JudgeIsSame2DArray(const Array<Array<T> > &a, const Array<Array<T> > &b) {
	if (a.size() != b.size()) {
		std::cerr << COLOR_RED "Error: Not the same array" COLOR_END << std::endl;
		exit(EXIT_FAILURE);
	}
...
static void
Print2DArray(const std::string &title, const Array<Array<int> > &array2D) {
